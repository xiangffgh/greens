...
package com.xiangff.greens.app.view;
...
//    }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        TextView addButton = (TextView) findViewById(R.id.numberbutton_add);
        addButton.setOnClickListener(this);
        TextView subButton = (TextView) findViewById(R.id.numberbutton_sub);
    ...
    public void onClick(View v) {
        int id = v.getId();
        int count = getNumber();
        if (id == R.id.numberbutton_sub) {
            if (count > 1) {
                //正常减
                mCount.setText("" + (count - 1));
            }

        } else if (id == R.id.numberbutton_add) {
        ...
        if (this.getTag()!=null){
            int position=Integer.parseInt(this.getTag().toString());
            Car.getInstance().setItemNum(Car.getInstance().getItems().get(position), value);
            if (textChangeListener!=null){
                textChangeListener.afterTextChanged(position,value);
            }
        }
    }
